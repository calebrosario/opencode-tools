...
const configSchema = z.object({
  ...
  WEBHOOK_TIMEOUT_MS: z.number().default(5000),
  ALERT_WEBHOOK_ENABLED: z.boolean().default(false),
...
export const CONFIG = configSchema.parse({
  ...
  MCP_MAX_CONNECTIONS: process.env.MCP_MAX_CONNECTIONS
    ? parseInt(process.env.MCP_MAX_CONNECTIONS)
    : undefined,
  MCP_REQUEST_TIMEOUT_MS: process.env.MCP_REQUEST_TIMEOUT_MS
    ? parseInt(process.env.MCP_REQUEST_TIMEOUT_MS)
    : undefined,
  ...
  LOG_MAX_FILES: process.env.LOG_MAX_FILES
    ...
    : undefined,
  CONTAINER_MEMORY_MB: process.env.CONTAINER_MEMORY_MB
    ? parseInt(process.env.CONTAINER_MEMORY_MB)
    : undefined,
  CONTAINER_CPU_SHARES: process.env.CONTAINER_CPU_SHARES
    ? parseInt(process.env.CONTAINER_CPU_SHARES)
    : undefined,
  CONTAINER_PIDS_LIMIT: process.env.CONTAINER_PIDS_LIMIT
    ? parseInt(process.env.CONTAINER_PIDS_LIMIT)
    : undefined,
  WEBHOOK_URL: process.env.WEBHOOK_URL,
  WEBHOOK_TIMEOUT_MS: process.env.WEBHOOK_TIMEOUT_MS
    ? parseInt(process.env.WEBHOOK_TIMEOUT_MS)
    : undefined,
  ALERT_WEBHOOK_ENABLED: process.env.ALERT_WEBHOOK_ENABLED === "true",
  ALLOW_PRIVILEGED_CONTAINERS:
    process.env.ALLOW_PRIVILEGED_CONTAINERS === "true",
