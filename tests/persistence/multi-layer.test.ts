...
describe('MultiLayerPersistence', () => {
  ...
  describe('Layer 2: JSONL logs', () => {
    ...
    test('should load logs with filters', async () => {
      ...
      const infoLogs = await multiLayerPersistence.loadLogs(testTaskId, { level: 'info' });
      const errorLogs = await multiLayerPersistence.loadLogs(testTaskId, { level: 'error' });
  ...
  describe('Layer 4: checkpoints', () => {
    ...
    test('should restore from checkpoint', async () => {
      ...
      // Create checkpoint
      const checkpointId = await multiLayerPersistence.createCheckpoint(testTaskId);
